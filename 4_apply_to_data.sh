...
# python3 $diploShicPath fvecVcf -h
# It is a bit involved so for now we are going to skip this step:
# python3 $diploShicPath fvecVcf haploid $vcfFile 2 243199373 $fvecFile --ancFileName preCookedData/chr2.anc.fa &> realfvec.log

# Instead, let's cheat
zcat preCookedData/$fvecFile.gz > $fvecFile

python3 $diploShicPath predict clf.json clf.weights.hdf5 $fvecFile real_preds.txt &> real_preds.log
